...
public class ReadWriteMySQLIntegrationTest implements BeforeAllCallback, ExtensionContext.Store.CloseableResource {
    ...
    public static void setProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("jdbc.config.user.base-packages[0]", () -> "io.github.opensabe.common.mybatis.test");
        ...
        registry.add("jdbc.config.user.data-source[1].cluster-name", () -> "public");
        registry.add("jdbc.config.user.data-source[1].driver-class-name", () -> "com.mysql.cj.jdbc.Driver");
        registry.add("jdbc.config.user.data-source[1].is-write-allowed", () -> false);
        registry.add("jdbc.config.user.data-source[1].name", () -> "user-2");
        registry.add("jdbc.config.user.data-source[1].min-idle", () -> 1);
        registry.add("jdbc.config.user.data-source[1].initial-size", () -> 1);
        registry.add("jdbc.config.user.data-source[1].max-active", () -> 2);
        registry.add("jdbc.config.user.data-source[1].username", () -> "root");
        registry.add("jdbc.config.user.data-source[1].password", () -> CustomizedMySQLContainer.MYSQL_ROOT_PASSWORD);
        registry.add("jdbc.config.user.data-source[1].url", () -> "jdbc:mysql://" + MYSQL_READ.getHost() + ":" + MYSQL_READ.getMysqlPort() + "/test");
